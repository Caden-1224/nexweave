...
struct DataEvent { std::uint32_t version=1; std::string request_id,session_id; std::uint64_t generation=0,sequence=0; DataEventType type=DataEventType::kError; std::string text; std::uint32_t frame_index=0; bool end=false; domain::ErrorCode error_code=domain::ErrorCode::kNone; std::string message; std::size_t expected_pcm_bytes=0; std::vector<std::uint8_t> pcm; };
