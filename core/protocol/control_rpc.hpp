...
struct ControlRequest { std::uint32_t version=1; std::string request_id, operation, work_id, session_id; std::uint64_t generation=0; std::chrono::milliseconds deadline{0}; };
// 响应始终回显 request_id；replayed 表示幂等缓存直接重放，调用方不得再次启动任务。
struct ControlResponse { std::uint32_t version=1; std::string request_id; domain::OperationResult result{}; bool replayed=false; };
