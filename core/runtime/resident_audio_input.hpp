...
//   - 读设备期间不持锁：真实声卡的 read 可能阻塞到它自己声明的超时。持锁读会把一次
//     设备阻塞放大成“消费者也取不到段”——消费者侧的等待与取段都要进同一把锁，因此在
//     一个长读窗口里会一起卡住。生产者侧既然是串行的，这里就不需要为并发的 stop()
//     留出窗口；pump 在每次读之前重新检查终态，是为了在一次预算内多次读设备时尽快
//     响应已经发生的结束或停止，而不是为了跨线程竞争。
