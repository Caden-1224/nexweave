...
const char* event_name(SessionStateMachine::Event e) {
  switch (e) {
    case SessionStateMachine::Event::kAudioStart:
      return "audio_start";
    case SessionStateMachine::Event::kAsrFinal:
      return "asr_final";
    case SessionStateMachine::Event::kRouteL0L1:
      return "route_l0_l1";
    case SessionStateMachine::Event::kRouteL2L3:
      return "route_l2_l3";
    case SessionStateMachine::Event::kLlmDone:
      return "llm_done";
    case SessionStateMachine::Event::kTtsDone:
      return "tts_done";
    case SessionStateMachine::Event::kCancel:
      return "cancel";
    case SessionStateMachine::Event::kCancelComplete:
...
domain::OperationResult SessionStateMachine::dispatch(Event e, std::uint64_t supplied_generation) {
  ...
  if (supplied_generation != generation_) {
    return domain::OperationResult::failure(domain::ErrorCode::kInvalidInput, "拒绝过时代际事件");
  }
  State next = state_;
  bool valid = false;
  switch (state_) {
    ...
    case State::kRouting:
      ...
      if (e == Event::kRouteL0L1)
        next = State::kSpeaking;
      if (e == Event::kRouteL2L3)
  ...
  const auto transition =
      std::string(state_to_text(state_)) + "--" + event_name(e) + "-->" + state_to_text(next);
