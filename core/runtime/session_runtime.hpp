...
struct SessionTurnResult {
  ...
  // 本轮的输入音频是否已经全部逐帧提交给识别。整段音频送完（含最后一次送帧里交付 final）
  // 之后为真；取消在送帧途中被受理时为假——那时还有帧没有送出去，“输入已完成”并不成立。
  // 它与“识别是否产出文本”无关：空文本与全空白文本同样是输入已提交。
  bool input_done = false;
  bool playback_started = false;
  bool playback_done = false;
  // 轮次收尾时播放清理（播放组件 stop()）的结果。ok 表示待播队列已经按约定丢弃，也就是
  // 取消/失败响应所承诺的“已经不会再发声”确有依据；非 ok 表示清理本身失败，此时终态不变
  // （本轮确实已经被取消或失败），但“已经静默”这一事实不成立，调用方必须按设备故障处理，
  // 不能把它当成一次干净取消。只记录第一处失败，避免收尾里的第二次 stop() 覆盖根因。
...
//
// 取消语义：所有取消走同一条收敛路径，但**受理时刻所处的阶段**决定了要做哪些事。阶段入口
// 共四个，它们不是取消来源的清单（来源见下一段）：
//   1. Listening：送帧循环在每个帧边界检查停止标志；受理后立即停止继续送帧，在能力回调
//      之外调用 asr_.cancel() 通知识别退出本轮，然后直接收敛——既不提交 AsrFinal 迁移，
//      也不检索、不生成、不合成，因此“取消”绝不会退化成一次知识问答。
//   2. Routing：路由是同步值计算且内部没有任何回调，所以不存在“路由执行中被取消”的注入
//      点，唯一可达的受理点是它的边界（识别 final 回调所在的那次 feed）。受理后不再提交
//      任何路由决策，因此由该决策导出的提示词、直答与合成都不会开始。
//   3. Thinking/Speaking：向**正在执行**的后端传播 cancel()（L2/L3 的 LLM 与 TTS），使后端
//      停止计算，而不是继续产出注定被丢弃的结果。该传播发生在投递回调内，因此依赖
//      ILlm/ITts 的“cancel() 是唯一允许在回调中重入的入口”约定，见 capability/backend.hpp；
//      识别侧不允许回调内重入，故一律由送帧循环在回调之外调用 asr_.cancel()。
//   4. 播放清理：丢弃播放组件里尚未播完的排队帧（已写出的声音不可撤回），解除停止标志借用，
//      由 finish_turn 产生唯一取消终态。
// 取消来源（谁请求了取消）与阶段正交，共有四类，它们都以取消终态收敛并共用上面的路径：
// 设备或能力组件回调里的停止指令、停止类控制意图（L0 的 stop/cancel）、用户新语音打断、
// 以及收尾之后对 Idle 会话的取消请求（最后一类只是幂等空操作，不产生新终态）。只有新语音
// 打断会在结果里携带归属，使“回答被打断”和“用户喊停”在证据里可区分。取消不撤回已经交付
// 的 PCM，也不删除常驻输入流；停止类控制意图在本轮不合成任何音频，因此不会把“停止”变成
// 知识问答，也不会为新回答留下残留输出。
// 终态裁决顺序固定为“背压 → 取消 → 生成/合成失败 → 设备错误 → 无输出 → 未播完”：取消一旦
// 受理，本轮就已经是取消，被取消的后端随后返回的 kCancelled 或失败只是取消的结果而不是独立
// 故障；顺序若反过来，一次正常取消会被报成后端错误，调用方也就无法区分“用户停止”和“节点坏掉”。
//
// 清理结果的报告方式：本版的播放清理是同步且非阻塞的（IAudioPlayback::stop 幂等、不等待），
// 因此取消路径**没有超时分支**——“清理完成”由两件事共同表达：唯一的取消终态，以及
// playback 的 pending_count() 归零。清理本身失败时终态不变，但失败原因会写进
// SessionTurnResult::cleanup_error，因为“清理失败”意味着“已经不会再发声”这一承诺没有兑现，
// 不能被静默吞掉。真实设备需要有限等待时，等待预算与超时错误码由适配器在自己的 stop() 内部
// 声明，会话不代它等待，也不把它的等待伪装成自己的收敛时间。
//
...
class SessionRuntime final {
 public:
  ...
  // 请求取消当前轮次：置位停止标志、把停止传播给正在执行的后端，并把状态机推进到
  // Cancelling。返回后不再有新的帧被提交给播放，也不会再启动新的合成；已在途的那一帧
  // 至多完成本次交付（并发窗口，无法撤回）。重复调用的语义分两种：状态机已经处于 Idle 时
  // 是幂等空操作（成功返回、不迁移、不推进代际）；已经处于 Cancelling 时第二次调用会因为
  // 迁移非法而返回 kInvalidInput——它如实表达“本次调用没有改变任何状态”，并且不会产生
  // 第二个终态。合法调用来源见类注释“线程与并发约定”。
 ...
 private:
  ...
  // 把“取消受理”传播给正在执行的后端，使它们停止计算而不是继续产出注定被丢弃的结果。
  // 只通知本轮确实在途的调用：llm_round_active_/tts_round_active_ 为假时不触碰对应后端，
  // 因此空转的取消不会给无关后端留下副作用。每轮至多传播一次（execution_cancel_notified_
  // 去重），因为后端 cancel() 只允许在回调中重入一次就足够封锁本轮，重复调用只会增加
  // 与取消无关的副作用面。识别侧刻意不在这里处理：IAsr 不允许在 feed 回调中重入，
  // 它由送帧循环在回调之外调用 asr_.cancel()。
  // 不阻塞、不分配、不抛异常；可在任意投递回调内被调用。
  void notify_execution_cancel() noexcept;
  ...
  // 是语义的一部分（背压 → 取消 → 生成/合成失败 → 设备错误 → 无输出 → 未播完），两条路径
  // 必须一致，否则同一批音频会因为走哪条路由而得到不同的错误码。取消之所以排在失败之前：
  // 被取消的后端随后返回的 kCancelled 是取消的结果而不是独立故障，顺序反过来会把一次
  // 正常取消报成后端错误。
  ...
  // 取消传播的簿记，全部是单轮内有效的标量，每轮开始时复位：
  //   - llm_round_active_/tts_round_active_ 表示“本轮的这一次后端调用还没返回”；
  //   - execution_cancel_notified_ 保证一次取消只向后端传播一次，避免重复调用把“取消”
  //     放大成多次后端副作用。
  // 三者都是原子量：取消可以由设备或能力组件的回调发起，而按上面的并发约定，那条回调未必
  // 与 run() 同线程，普通 bool 的读写在那种调用下是数据竞争；去重也必须用 exchange 而不是
  // “先读后写”，否则两次并发取消会同时通过判断。复位发生在 run() 的单线程阶段，此时不可能
  // 有并发取消，因此用 store 即可。
  std::atomic<bool> llm_round_active_{false};
  std::atomic<bool> tts_round_active_{false};
  std::atomic<bool> execution_cancel_notified_{false};
