 ...
 * 新请求和有效取消各推进一代，旧代事件在提交前被拒绝。代际达到 uint64 上限时，
 * 开始或取消返回 kInvalidInput，不回绕。字符串/轨迹分配异常向上传播；提交先构造轨迹，
 * 再做无抛出赋值，因此失败时状态、代际和轨迹均保持原值，锁由 RAII 自动释放。
 */
class SessionStateMachine final {
 public:
  ...
  // 仅用于同步控制事件，异步回调必须使用显式代际重载，不能把旧结果重新标为当前代。
  ...
  State state() const;
  // 返回当前代际快照；调用方应在启动异步能力前保存它并原样传回 dispatch。
