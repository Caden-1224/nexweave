 ...
 * (起始相位 + g) % 80 < 40 ? +8000 : -8000 决定；起始相位 =
 * (UTF-8 字节和 + 31 × 位置加权字节和) % 80，帧数 = ⌈字节数 / 16⌉。
 * 同一文本每次合成得到逐采样一致的 PCM。不同文本的差异来自帧数与起始相位两处；
 * 相位只有 80 个取值，因此这是“不同文本通常可区分”的确定性构造，**不是**文本到波形的
 * 唯一映射——等长且相位相同的文本仍会得到相同波形，用它论证隔离时不能超出这一点。
