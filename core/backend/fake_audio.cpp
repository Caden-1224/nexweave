...
domain::OperationResult FakeAudioSource::open() {
  ...
  if (pcm_samples_.empty() || pcm_samples_.size() % domain::kAudioFrameSamples != 0) {
    return failure(domain::ErrorCode::kInvalidInput, "PCM 必须包含至少一帧且长度为 320 的整数倍");
  }
  next_sample_ = 0;
  cancelled_ = false;
  opened_ = true;
...
domain::Result<domain::AudioFrame> FakeAudioSource::read() {
  ...
  const auto begin = pcm_samples_.begin() + static_cast<std::ptrdiff_t>(next_sample_);
  const auto end = begin + static_cast<std::ptrdiff_t>(domain::kAudioFrameSamples);
  const std::vector<std::int16_t> frame_samples(begin, end);
  const auto frame = domain::AudioFrame::from_samples(frame_samples);
  if (!frame.ok()) {
    return domain::Result<domain::AudioFrame>::failure(domain::ErrorCode::kInvalidInput,
                                                       "Fake 音频帧不满足固定合同");
  }
  // 先完成返回值的样本复制再推进游标，分配失败不能静默丢掉一帧。
  auto result = domain::Result<domain::AudioFrame>::success(frame.value);
...
domain::OperationResult FakeAudioSink::write(const domain::AudioFrame& frame) {
  ...
  if (!domain::validate_audio_frame(frame).ok()) {
    return failure(domain::ErrorCode::kInvalidInput, "输出帧不满足固定音频合同");
  }
