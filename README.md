...
> **当前处于早期开发阶段。** 已实现基础契约、会话状态机、确定性 Fake 音频输入输出与 Fake 流式 ASR。完整 Session、网络服务、真实模型和板端语音交互尚未交付。
...
目前只有 Fake 音频和 Fake ASR 已实现完整的相应模拟后端。其他 Fake 能力、真实检索和板端后端仍待实现。
...
| Fake RAG、LLM、TTS 与完整 Session | 规划中 | 流式重叠、持续输入和端到端取消 |
